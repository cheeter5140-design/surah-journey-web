...
type LocalizedSurahContent = { name: string; meaning: string; ayahTranslations?: string[] };

const EN_SURAH_CONTENT: Record<number, LocalizedSurahContent> = {
  1: {
    name: "Al-Faatiha",
    meaning: "The Opening",
    ayahTranslations: [
...
export function getLocalizedSurahMeta(quranNumber: number, lang: Lang): { name?: string; meaning?: string } {
  if (lang !== "en") return {};
  const content = EN_SURAH_CONTENT[quranNumber];
  return content ? { name: content.name, meaning: content.meaning } : {};
}
...
export function getLocalizedSurahs(lang: Lang): Surah[] {
  return lang === "en" ? SURAHS.map((surah) => localizeSurah(surah, lang)) : SURAHS;
}

export function getLocalizedSurahById(surahId: number, lang: Lang): Surah | undefined {
